=== CBIS Sync ===
Contributors: Jonas Hjalmarsson (Hultsfreds kommun)
Tags: cbis, citibreak, rss, sync
Requires at least: 3.2
Tested up to: 3.5
Stable tag: trunk
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

CBIS plugin to sync from a Citibreak database to Wordpress posts made by Hultsfreds kommun.

== Description ==

This plugin uses Citibreak Information System (CBIS) to sync selected information to local cached posts.

Contact Jonas Hjalmarsson, Hultsfreds kommun on Twitter @hjalmarsson for questions.


== Installation ==

Download to your WordPress-installation or download and unzip manually to the plugins-folder.

Activate in Plugins section.

Visit the settings page to setup and enable the sync.


== Screenshots ==


== Changelog ==

= 0.9 =
Some text